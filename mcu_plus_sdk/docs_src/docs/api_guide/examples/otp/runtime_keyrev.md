...
\note Set this macro **UPDATE_KEYREV** in the `{SDK_PATH}/examples/otp/runtime_keyrev/{BOARD}/r5fss0-0_nortos/main.c` to update the KEY Revision.
...
- After successful flashing, power off the board and set the bootmode to OSPI NAND \ref BOOTMODE_OSPI_NAND.
...
- After successful flashing, power off the board and set the bootmode to OSPI \ref BOOTMODE_OSPI.
- Power on the board. The example should boot with the logs appearing on WKUP_UART0.
