...
\cond SOC_J722S
This example demonstrates multi-core ethernet support on J722S using CPSW. The server runs on MAIN-R5F core and manages CPSW peripheral, while client applications on MCU-R5F core communicate via shared memory IPC.
\endcond
...
\cond SOC_J722S
 Parameter      | Value
 ---------------|-----------
 CPU + OS       | main-r5fss0-0_freertos
 Toolchain      | ti-arm-clang
 Board          | @VAR_BOARD_NAME_LOWER
 Example folder | source/networking/ethfw/apps/app_remoteswitchcfg_server/j722s/
\endcond
...
\cond SOC_J722S
 ...
 Toolchain      | ti-arm-clang
 Board          | @VAR_BOARD_NAME_LOWER
 Example folder | source/networking/ethfw/apps/app_remoteswitchcfg_client/j722s/
\endcond
...
\endcond
\cond SOC_J722S
- Connect one end of the CAT6 cable to the EVM and the other end to the network.
\endcond
## Run the example
...
\endcond
\cond SOC_J722S
- Load main-r5 Server binary in CCS in main-r5f core after doing a power/system reset.
- Load the other binaries via CCS on mcu-r5f core as per need
\endcond
\cond SOC_AM62PX || SOC_AM62DX
...
\endcond
\cond SOC_J722S
### MAIN-R5 Server Application logs
\code
Enabling clocks!
=======================================================
            CPSW Ethernet Firmware                     
...
ETHFW Version   : 0.05.00
ETHFW Build Date: May  8, 2026
ETHFW Build Time: 16:29:47
ETHFW Commit SHA: 
...
cb_lld_task_create: Uniconf Task stack_size=16384
cb_rawsock_open:combase-1.1.3
IETHFW: ALLOC_RX | C2S | core=0 endpt=36 token=2600
ETHFW: ALLOC_RX | S2C | flow=20,5 rxPsil=0x4600 status=0
s=1 dmaRxChId=-1 nTxPkts=0 nRxPkts=0 pktSize=0
cbl_query_response:tilld1 link DOWN !!!!
cbl_query_response:tilld2 link DOWN !!!!
cb_lld_task_create: gPTP Task stacETHFW: ALLOC_MAC | C2S | core=0 endpt=36 token=2600
ETHFW: ALLOC_MAC | S2C | macAddr=70:ff:76:1d:ec:e4 status=0
max_ports=2
cb_rawsock_open:combase-1.1.3
cb_rawsock_open:dmaTxChId=1 numRxChannels=1 dmaRxChId=2 nTxPkts=16 nRxPkts=16 pktSize=1536
rxChId 2 has owner dmaRxSharedETHFW: REGISTER_MAC | C2S | core=0 endpt=36 token=2600 macAdd=70:ff:76:1d:ec:e4 flowIdx=20,5
Cpsw_internalIoctl_handler_ENET_IOCTL_REGISTER_DSTMAC_RX_FLOW:432 
v:tilld2 open success
gptpnet_init:supportRtNotice=0 tout_interval=125000000Ns
gptpnet_init:Open lldtsync OK!
IEEE1588-2019 performance monitoring disabled.
INF:gptpETHFW: REGISTER_MAC | S2C | status=0
current-log-gptp-cap-interval=3 initial-log-gptp-cap-interval=3
pp_glb_init: use-mgt-log-gptp-cap-interval=0
current-log-gptp-cap-interval=3 initial-log-gptp-cap-intervalerval=0
current-log-gptp-cap-interval=3 initial-log-gptp-cap-interval=3
...
syncIntervalNs=125000000, announceInterval=0,pdelayReqInterval=10enet_activate:tilld1 status=0, duplex=1, speed=0Mbps
onenet_activate:tilld2 status=0, duplex=1, speed=0Mbps
GPTP_MEDIUM_ALLOC: fragsize=16 fragused/fragnum=853/944 (90)
INF:ub=19/74 (25)
SM_DATA_INST: fragsize=8 fragused/fragnum=2032/2032 (100)
domainIndex=0, GM changed old=00:00:00:00:00:00:00:00, new=70:FF:76:FF:FE:1D:EC:F2
INF:gptpnIndex=0, gmstate=2
Failed to get link info: -1
cbl_query_response:failed to get speed and duplex : tilld2
index=2 speed=0, duplex=unknown UP, speed
!!! Full duplex link with Speed above 100 Mbps needed for gptp to run !!!
[0]Network Link UP Event
Cpsw_handleLinkUp:1423 
MAC Port 1: link up
cbl_query_response:tilld1: link UP, speed=1000, duplex=1 !!!! (170us since link change event)
index=1 speed=1000, duplex=full
ETHFW: REGISTER_IPv4 | C2S | core=0 endpt=36 token=2600 ipAddr=192.168.1.14 macAdd=70:ff:76:1d:ec:e4
ETHFW: 
        SNo.      MAC Address        VLAN     IP Address
ETHFW: ------  -------------------  ------  -----------------
ETHFW:     1    70:ff:76:1d:ec:e4       0    192.168.1.14
ETHFW: REGISTER_IPv4 | S2C | status=0
Added interface 'br3', IP is 192.168.1.12
\endcode

### MCU-R5 Client Application logs
\code
CpswProxy: Local cmd endpt 36, notify endpt 30
CpswProxy: ETHFW services found at core 2 endpts 34 (ti.ethfw.ethdevice) and 24 (ti.ethfw.notifyservice)
Starting lwIP, local interface IP is dhcp-enabled
[LWIPIF_LWIP] NETIF INIT SUCCESS
CpswProxy: ATTACH | C2S | virtPort=26
CpswProxy: ATTACH | S2C | token=2600 rxMtu=1522 features=3 numTxCh=1 numRxFlow=1 status=0
...
CpswProxy: ALLOC_RX | S2C | token=2600 flow=20,5 rxPsil=0x4600 status=0
CpswProxy: ALLOC_MAC | C2S | token=2600
CpswProxy: ALLOC_MAC | S2C | token=2600 macAddr=70:ff:76:1d:ec:e4 status=0
CpswProxy: REGISTER_MAC | C2S | token=2600 flowIdx=20,5
CpswProxy: REGISTER_MAC | S2C | token=2600 status=0
Host MAC address : 70:ff:76:1d:ec:e4
[LWIPIF_LWIP] Enet has been started successfully
Added interface 'ti0', IP is 0.0.0.0
[0]Network Link UP Event
Added interface 'ti0', IP is 192.168.1.14
CpswProxy: REGISTER_IPv4 | C2S | token=2600 ipAddr=192.168.1.14 macAdd=70:ff:76:1d:ec:e4
CpswProxy: REGISTER_IPv4 | S2C | token=2600 status=0
\endcode
\endcond
\cond SOC_AM62PX || SOC_AM62DX
...
The Ethernet firmware on J722S is configured with the following default port settings:
- **MAC Port 1**: Enabled in RGMII mode with integrated PHY support
- **MAC Port 2**: Disabled (NO-PHY mode) by default

MAC Port 2 can be configured by the user to operate in SGMII mode for SerDes-based connectivity. However, since integrated PHY support for SGMII is not available on J722S, MAC-to-MAC mode must be employed. This mode enables direct connection between J722S and another Jacinto device (such as J721E or similar) through an Ethernet Expander Bridge, allowing SGMII functionality via the remote device's SGMII port.

## Enabling SGMII Mode on MAC Port 2

To configure SGMII operation on MAC Port 2 of the J722S Ethernet firmware, perform the following steps:

1. **Enable SGMII in board configuration** - Modify `<SDK_INSTALL_PATH>/source/networking/ethfw/utils/board/src/j722s/board_j722s_evm.c`:
   - Set `#define ENABLE_SGMII` to `1`

2. **Enable SGMII in SoC configuration** - Modify `<SDK_INSTALL_PATH>/source/networking/enet/soc/k3/j722s/enet_soc_cfg.h`:
   - Set `#define ENET_CFG_CPSW_SGMII` to `(ENET_ON)`
   - Set `#define ENET_CFG_CPSW_MACPORT_SGMII` to `(ENET_ON)`

3. **Configure CPSW pinmux in SysConfig** - Update `example.syscfg` using the SysConfig GUI:
   - Navigate to `CPSW pinmux config`
   - Set `MAC Port 2 Interface Mode` to `SGMII` from the dropdown menu
\imageStyle{sgmii_enable_j722s.png,width:30%}
\image html sgmii_enable_j722s.png

4. **Establish board-to-board connection** - Connect the J722S board to a compatible Jacinto board (J721E or equivalent) using an Ethernet Expander Bridge:
   - Connect the SGMII port of the remote Jacinto device to serve as the SGMII interface for J722S MAC Port 2

5. **Rebuild firmware and applications** - Rebuild the libraries first, then the apps before loading the binaries
